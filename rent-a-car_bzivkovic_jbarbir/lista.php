<?php
  ...
  if (isset($_GET["kategorija"])){
    ...
    $query="SELECT * FROM rac_automobili WHERE kategorija like '$kategorija%'";
    $res = mysqli_query($connection,$query);
    if (mysqli_num_rows($res)>0) {
        while ($row=mysqli_fetch_array($res)){;
		   $slika = "images/cars/".$row['proizvodjac']." ".$row['model'].".jpg";
		   if (strlen($row['slika'])>0) $slika = $row['slika'];
        ...
        echo "<div class='w3-container'>";
           ...
           <img src='".$slika."' style='width:100%' class='w3-round'>
